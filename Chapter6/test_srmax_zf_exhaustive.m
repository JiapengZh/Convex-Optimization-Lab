function test_srmax_zf_exhaustive(filename)
...
try
    for sysSize = 1:size(testSets,1)
        ...
        for iter=1:nIter
            for p = 1:length(Parr)
                [R, q] = srmax_zf_exhaustive(testSets(sysSize,iter).H,Parr(p));
                
                if max(abs(R-testSets(sysSize,iter).zf_ex_R(p))) > zeta
                    display(['FAIL, wrong result for sumrate, test No. ' num2str(iter) ',' num2str(p)])
                    keyboard
                    ok=false;
                end
                
                if max(abs(q-testSets(sysSize,iter).zf_ex_q(:,p))) > zeta
